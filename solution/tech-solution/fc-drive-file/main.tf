...
resource "alicloud_ram_role_policy_attachment" "fc_demo_policy_attachment" {
  role_name   = alicloud_ram_role.fc_demo_role.name
...
export ALIYUN_RAM_ROLE_NAME="${alicloud_ram_role.fc_demo_role.name}"
...
mode = EcsRamRole
ecsRoleName = ${alicloud_ram_role.fc_demo_role.name}
...
resource "alicloud_ram_role_policy_attachment" "oss_event_notification_policy" {
  count       = local.oss_role_exists ? 0 : 1
  role_name   = alicloud_ram_role.oss_event_notification_role[0].name
