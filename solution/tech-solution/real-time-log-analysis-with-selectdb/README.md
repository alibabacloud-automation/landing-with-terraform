## Introduction
<!-- DOCS_DESCRIPTION_CN -->
本示例用于实现解决方案[SelectDB实现日志高效存储与实时分析
](https://www.aliyun.com/solution/tech-solution/real-time-log-analysis-with-selectdb)，涉及专有网络（VPC）、交换机（VSwitch）、云服务器（ECS）、SelectDB数据库（SelectDB）等资源的部署。
<!-- DOCS_DESCRIPTION_CN -->
