...
|------|-------------|------|---------|:--------:|
| <a name="input_common_name"></a> [common\_name](#input\_common\_name) | 通用名称，用于资源命名的前缀。 | `string` | `"cache"` | no |
| <a name="input_db_instance_class"></a> [db\_instance\_class](#input\_db\_instance\_class) | RDS实例规格，根据redis类型和可用区支持的情况选择实例规格。 | `string` | `"mysql.n1e.medium.1"` | no |
...
| <a name="input_redis_class"></a> [redis\_class](#input\_redis\_class) | Redis规格，根据redis类型和可用区支持的情况填写实例规格。 | `string` | `"redis.shard.small.2.ce"` | no |
